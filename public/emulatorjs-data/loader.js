(async function() {
    ...
    const originalInitGameCore = EmulatorJS.prototype.initGameCore;
    EmulatorJS.prototype.initGameCore = async function(runtimeJs, runtimeWasm, runtimeWorker) {
        const core = this.getCore ? this.getCore() : config.system;
        const runtimeBase = scriptPath + "cores/runtime/" + core + "_libretro";

        if (!runtimeJs) {
            const response = await fetch(runtimeBase + ".js");
            if (!response.ok) throw new Error("Missing runtime JS: " + runtimeBase + ".js");
            runtimeJs = await response.text();
        }

        if (!runtimeWasm) {
            const response = await fetch(runtimeBase + ".wasm");
            if (!response.ok) throw new Error("Missing runtime WASM: " + runtimeBase + ".wasm");
            runtimeWasm = new Uint8Array(await response.arrayBuffer());
        }
